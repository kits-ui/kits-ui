<template>
  ...
  <template v-else>
    <div
      ...
      :class="[
        'menus-item',
        item.disabled ? 'menus-item-disabled' : 'menus-item-available',
        item.divided ? 'menus-divided' : null,
        !item.disabled && activeIndex === index ? 'menus-item-active' : null,
        menusItemClass,
      ]"
    >
      ...
      <div class="menus-item-suffix">
        <slot
          v-if="item.children && slots.suffix"
          name="suffix"
          :item="{ activeIndex, item }"
        ></slot>
        <template v-else-if="item.children">▶</template>
        <slot
          v-else-if="item.tip && slots.suffix"
          name="suffix"
          :item="{ activeIndex, item }"
        ></slot>
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    function menusClick(event, item) {
      ...
      if (item && !item.disabled && !item.hidden && typeof item.click === 'function') {
